# Import flask and datetime module for showing date and time
from flask import Flask,Response
import datetime
import mediapipe as mp
from flask_cors import CORS
import pyautogui
import time
import cv2
import face_recognition
import numpy as np


x = datetime.datetime.now()
app = Flask(__name__)
CORS(app)
camera = cv2.VideoCapture(0)

drawing = mp.solutions.drawing_utils
hands = mp.solutions.hands
hand_obj = hands.Hands(max_num_hands=1)


vijay_image = face_recognition.load_image_file("vijay.jpg")
vijay_face_encoding = face_recognition.face_encodings(vijay_image)[0]

revanth_image = face_recognition.load_image_file("revanth.jpg")
revnth_face_encoding = face_recognition.face_encodings(revanth_image)[0]

karthi_image = face_recognition.load_image_file("karthi.jpg")
karthi_face_encoding = face_recognition.face_encodings(karthi_image)[0]


sathish_image = face_recognition.load_image_file("sathish.jpg")
sathish_face_encoding = face_recognition.face_encodings(sathish_image)[0]

rishanth_image = face_recognition.load_image_file("rishanth.jpg")
rishanth_face_encoding = face_recognition.face_encodings(rishanth_image)[0]




known_face_encodings = [
    vijay_face_encoding,
    revnth_face_encoding,
    karthi_face_encoding,
    sathish_face_encoding,
    rishanth_face_encoding
]
known_face_names = [
    "vijay",
    "revanth kumar",
    "karthi",
    "sathish",
    "rishanth",
]
face_locations = []
face_encodings = []
face_names = []
process_this_frame = True


@app.route('/data')
def get_time():
    return {
        'Name':"geek", 
        "Age":"22",
        "Date":x, 
        "programming":"python"
        }


def gen_frames():  
    while True:
        success, frame = camera.read()  # read the camera frame
        frame = cv2.flip(frame,1)
        res = hand_obj.process(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))

        if res.multi_hand_landmarks:
            drawing.draw_landmarks(frame, res.multi_hand_landmarks[0], hands.HAND_CONNECTIONS)


        if not success:
            break
        else:
            # Resize frame of video to 1/4 size for faster face recognition processing
            small_frame = cv2.resize(frame, (0, 0), fx=0.25, fy=0.25)
            # Convert the image from BGR color (which OpenCV uses) to RGB color (which face_recognition uses)
            rgb_small_frame = small_frame[:, :, ::-1]

            # Only process every other frame of video to save time
           
            # Find all the faces and face encodings in the current frame of video
            face_locations = face_recognition.face_locations(rgb_small_frame)
            face_encodings = face_recognition.face_encodings(rgb_small_frame, face_locations)
            face_names = []
            for face_encoding in face_encodings:
                # See if the face is a match for the known face(s)
                matches = face_recognition.compare_faces(known_face_encodings, face_encoding)
                name = "Unknown"
                # Or instead, use the known face with the smallest distance to the new face
                face_distances = face_recognition.face_distance(known_face_encodings, face_encoding)
                best_match_index = np.argmin(face_distances)
                if matches[best_match_index]:
                    name = known_face_names[best_match_index]

                face_names.append(name)
            

            # Display the results
            for (top, right, bottom, left), name in zip(face_locations, face_names):
                # Scale back up face locations since the frame we detected in was scaled to 1/4 size
                top *= 4
                right *= 4
                bottom *= 4
                left *= 4

                # Draw a box around the face
                cv2.rectangle(frame, (left, top), (right, bottom), (0, 0, 255), 2)

                # Draw a label with a name below the face
                cv2.rectangle(frame, (left, bottom - 35), (right, bottom), (0, 0, 255), cv2.FILLED)
                font = cv2.FONT_HERSHEY_DUPLEX
                cv2.putText(frame, name, (left + 6, bottom - 6), font, 1.0, (255, 255, 255), 1)

            #xx

            ret, buffer = cv2.imencode('.jpg', frame)
            frame = buffer.tobytes()
            yield (b'--frame\r\n'
                   b'Content-Type: image/jpeg\r\n\r\n' + frame + b'\r\n')

            #xx


@app.route('/video_feed')    
def video_feed():  
    return Response(gen_frames(), mimetype='multipart/x-mixed-replace; boundary=frame')
 

if __name__ == '__main__':
    app.run(host="0.0.0.0",port=5000,debug=True)